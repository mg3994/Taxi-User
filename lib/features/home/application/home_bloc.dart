...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  ...
  double minChildSize = 0.47;
  double maxChildSize = 0.99;
  double sheetSize = 0.48;
  double previousScrollOffset = 0.0;
  double midChildSize = 0.67;
...
//  Locations
  ...
  FutureOr<void> getAutoCompleteSearchPlaces(
    ...
    if (event.searchText.length >= 4) {
      ...
      if (recentSearchPlaces.any((element) => element.address
          .toLowerCase()
          .contains(event.searchText.toLowerCase()))) {
        autoSearchPlaces.clear();
        searchInfoMessage = AppLocalizations.of(event.context)!.searchResult;
        for (var i = 0; i < recentSearchPlaces.length; i++) {
          if (recentSearchPlaces[i]
              .address
              .toLowerCase()
              .contains(event.searchText.toLowerCase())) {
            autoSearchPlaces.add(recentSearchPlaces[i]);
          }
        }
        emit(SearchOnChangeState());
      } else {
            ...
            if (event.mapType == 'google_map') {
              for (var element in success) {
                autoSearchPlaces.add(AddressModel(
                    ...
                    lat: 0,
                    lng: 0,
                    isAirportLocation: (element['description']
                            .toString()
                            .toLowerCase()
                            .contains('airport'))
                        ? true
              ...
              for (var element in success) {
                autoSearchPlaces.add(AddressModel(
                    ...
                    isAirportLocation: (element['display_name']
                            .toString()
                            .toLowerCase()
                            .contains('airport'))
                        ? true
  ...
  // A method to update the minChildSize, if needed
  void updateMinChildSize(double newMinSize) {
    minChildSize = newMinSize;
  }

  // A method to update the maxChildSize, if needed
  void updateMaxChildSize(double newMaxSize) {
    maxChildSize = newMaxSize;
  }
